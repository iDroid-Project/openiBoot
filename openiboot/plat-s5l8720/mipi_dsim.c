...
int mipi_dsim_init(LCDInfo* LCDTable) {
	...
	if (mashFest) {
		...
		SET_REG(MIPI_DSIM + CLKCTRL, CLKCTRL_ESC_PRESCALER(LCDTable->unkn18 >> 4)
			| CLKCTRL_ESC_CLKEN | CLKCTRL_PLL_BYPASS | CLKCTRL_BYTE_CLK_SRC);
		SET_REG(MIPI_DSIM + PLLCTRL, (LCDTable->unkn18 << 16) & 0xF000000);
		SET_REG(MIPI_DSIM + SWRST, SWRST_RESET);
	}
	...
	SET_REG(MIPI_DSIM + MSYNC, MSYNC_HSPW(LCDTable->horizontalSyncPulseWidth) | MSYNC_VSPW(LCDTable->verticalSyncPulseWidth));
	...
	SET_REG(MIPI_DSIM + CONFIG, 0x7807027);	//TODO: write that in terms of constants
	SET_REG(MIPI_DSIM + CLKCTRL, GET_REG(MIPI_DSIM + CLKCTRL) | CLKCTRL_BYTE_CLKEN
		| CLKCTRL_ESC_EN_CLK | CLKCTRL_ESC_EN_DATA(DATA_LANES_ENABLED));
	SET_REG(MIPI_DSIM + FIFOCTRL, FIFOCTRL_MAIN_DISP_FIFO | FIFOCTRL_SUB_DISP_FIFO
		| FIFOCTRL_I80_FIFO | FIFOCTRL_TX_SFR_FIFO | FIFOCTRL_RX_FIFO);
